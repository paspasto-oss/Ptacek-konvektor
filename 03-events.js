...
$('rememberCodes').addEventListener('click',()=>{state.items.forEach(it=>{if(it.pohodaCode.trim()){if(it.vendorCode)mappings['vendor:'+it.vendorCode]=it.pohodaCode.trim();if(it.ptacekNo)mappings['ptacek:'+it.ptacekNo]=it.pohodaCode.trim();}});saveMappings();renderRows();refresh();alert('Mapovanie kódov bolo uložené v tomto prehliadači.');});
